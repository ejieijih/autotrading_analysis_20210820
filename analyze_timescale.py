    ...
    if (alg == 'linear_regression') or (alg == 'search_all'):
        ...
        for bs_i, [bs, bs_index] in enumerate(zip(['buy','sell'],[is_buy,~is_buy])):
            ...
            for pc_i in range(pc_num):
                ...
                n,bins,patches = plt.hist(Xd[:,pc_i],30,alpha=0.6,color='g',label='histgram')

                plt.close('all')
                plt.figure()
                plt.bar(bins[:-1],n*(100/max(n)),width=bins[1:]-bins[:-1],alpha=0.6)
                plt.scatter(
                    Xd[:,pc_i],
                    y,
                    label='samples',
                ...
                if alg == 'linear_regression':
                    ...
                    plt.plot(x_fit[:,bs_i,pc_i,:].T,y_fit[:,bs_i,pc_i,:].T[0]*10,color='r')
                    plt.plot(x_fit[:,bs_i,pc_i,:].T,y_fit[:,bs_i,pc_i,:].T[1]*10,color='g')
                ...
                print('saving: {}'.format(str(fname)))
                plt.savefig(str(fname))
    elif alg in ['multiple_regression','ridge','lasso']:
